...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/asr_hi_bn_en_step1",
        help="Where to write summaries and regenerated completions.",
    )
    parser.add_argument(
        "--generate-missing",
        action="store_true",
        help="Generate missing completion files from the base models.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=8,
        help="Generation batch size when creating missing completions.",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
        default=256,
        help="Maximum new tokens for generation when creating missing completions.",
    )
    parser.add_argument(
        "--sample-size",
        type=int,
        default=0,
        help="Optional sample size per language. 0 means use the full available dataset or completion file.",
    )
...
def evaluate_model_language_pairs(args: argparse.Namespace) -> Tuple[Dict, List[Dict]]:
    ...
    print(f"\n{'='*70}")
    print("EVALUATION: WildGuard & Gemma-3-27B-it (Zero-shot)")
    print(f"{'='*70}\n")
    ...
    for model_name, model_path in MODELS.items():
        ...
        for lang_idx, lang in enumerate(LANGUAGES, 1):
            print(f"\n  [{lang_idx}/{len(LANGUAGES)}] Language: {lang.upper()}")
            pair_start = time.time()

            completions, source_path = resolve_completion_set(model_name, model_path, lang, args)
            if not completions:
                print("      Skipping - no completion file and generation disabled")
                continue
            ...
            print("      Evaluating with WildGuard...")
            wg_start = time.time()
            wildguard_eval = wildguard.evaluate_all(
                completions=[dict(item) for item in completions],
                logger=None,
                cfg=type("Cfg", (), {"lang": lang})(),
            )
            ...
            print("      Evaluating with Gemma-3-27B-it...")
            g3_start = time.time()
    ...
    print(f"\n{'='*70}")
    print(f"✓ EVALUATION COMPLETE! Total time: {total_time/60:.1f} minutes ({total_time:.0f}s)")
    print(f"{'='*70}\n")
